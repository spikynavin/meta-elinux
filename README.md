# Meta-elinux layer for Raspberry pi 3b+

The meta layer for Raspberry pi 3b+ (kirkstone, dunfell) yocto build.
...
```mermaid
graph
A(meta-elinux) -- Layer added in bblayer conf --> B(layer contains build conf) --> C(Yocto kirkstone/dunfell baseline)
